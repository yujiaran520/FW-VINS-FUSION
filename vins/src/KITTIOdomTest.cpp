...
int main(int argc, char** argv)
{
	...
	{	
		if(rclcpp::ok())
		{
			...
			const int64_t image_time_ns = static_cast<int64_t>(
				std::llround(imageTimeList[i] * 1e9));
			imLeftMsg->header.stamp = rclcpp::Time(image_time_ns);
			...
			imRightMsg->header.stamp = rclcpp::Time(image_time_ns);
