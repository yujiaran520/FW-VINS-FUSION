...
void publish_car_model(double t, Eigen::Vector3d t_w_car, Eigen::Quaterniond q_w_car)
{
    ...
    car_mesh.header.stamp = timeFromSeconds(t);
...
{
    //printf("vio_callback! \n");
    double t = stampToSeconds(pose_msg->header.stamp);
    last_vio_t = t;
...

    {
        ...
        {
            const auto GPS_msg = gpsQueue.front();
            double gps_t = stampToSeconds(GPS_msg->header.stamp);
            ...
            if(gps_t >= t - 0.01 && gps_t <= t + 0.01)
            {
                ...
                Eigen::Vector3d position_stddev = Eigen::Vector3d::Ones();
                if (GPS_msg->position_covariance_type !=
                    sensor_msgs::msg::NavSatFix::COVARIANCE_TYPE_UNKNOWN)
                {
                    const int covariance_diagonal[3] = {0, 4, 8};
                    for (int axis = 0; axis < 3; ++axis)
                    {
                        const double variance =
                            GPS_msg->position_covariance[covariance_diagonal[axis]];
                        if (std::isfinite(variance) && variance > 0.0)
                            position_stddev(axis) = std::sqrt(variance);
                    }
                }
    ...
    globalEstimator.getGlobalResult(global_t, global_q, global_path);
    ...
    foutC.precision(0);
    foutC << stampToNanoseconds(pose_msg->header.stamp) << ",";
    foutC.precision(5);
    foutC << global_t.x() << ","
            << global_t.y() << ","
            << global_t.z() << ","
